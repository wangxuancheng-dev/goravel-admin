...
async function proxyWs(request, env, url) {
  ...
  // Non-upgrade probes: plain HTTP proxy (debugging / health).
  ...
  // Bridge: Worker opens WS to API, returns 101 + WebSocketPair to the browser.
  // Plain `return fetch(request)` often fails to complete browser WSS on Assets Workers.
