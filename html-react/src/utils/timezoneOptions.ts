...
export const PRESET_TIMEZONES: Array<{ value: string; label: string }> = [
...
export function formatOffsetLabel(tz: string): string {
  if (!tz) return ''
...
export function isPresetTimezone(tz: string): boolean {
  return PRESET_TIMEZONES.some((item) => item.value === tz)
}

/** Options for Select; appends unknown current value so edits stay valid. */
export function timezoneSelectOptions(current?: string) {
