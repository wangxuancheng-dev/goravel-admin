...
// 对话框标题（新增/编辑区分）
const dialogTitle = computed(() => formData.id ? t('allowlist.edit_allowlist') : t('allowlist.add_allowlist'))
...
const formRules = computed(() => ({
  ip: [
    { required: true, message: t('allowlist.ip_required'), trigger: 'blur' },
    {
      validator: (rule, value, callback) => {
        if (!value || value.trim() === '') {
          callback(new Error(t('allowlist.ip_required')))
          return
        }
        // 前端简单验证，后端会做详细验证
        ...
        for (const ip of ipList) {
          ...
          if (!trimmedIP.includes('.') && !trimmedIP.includes('/') && !trimmedIP.includes('-')) {
            callback(new Error(t('allowlist.ip_format_error')))
            return
          }
        }
        callback()
  ...
  const fields = [
    {
      ...
      rows: 4, // 行数和原代码一致
      placeholder: t('allowlist.ip_placeholder'),
      disabled: loading.value // 加载中禁用
    },
    {
      prop: 'remark',
      label: t('allowlist.remark'),
      type: 'textarea', // 备注的textarea
      rows: 3,
      placeholder: t('allowlist.remark_placeholder'),
...
const loadDetail = async (id) => {
  loading.value = true
  try {
    const res = await getAllowlistDetail(id)
    if (res.data && res.data.allowlist) {
      const allowlist = res.data.allowlist
      // 使用工具函数映射字段，自动处理 snake_case 和 PascalCase
      const mapped = mapFields(allowlist, getFormInitialValue())
      Object.assign(formData, mapped)
    }
  } catch (error) {
    console.error('Load allowlist detail error:', error)
  } finally {
    loading.value = false
  }
}
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        ...
        if (formData.id) {
          await updateAllowlist(formData.id, submitData)
          ElMessage.success(t('allowlist.update_success'))
        } else {
          const confirmCode = await promptSensitiveConfirm(t)
          await createAllowlist({ ...submitData, confirm_code: confirmCode })
          ElMessage.success(t('allowlist.create_success'))
        }
