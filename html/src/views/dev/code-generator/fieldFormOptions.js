...
export function isNumericFieldType(dbType) {
  return NUMERIC_TYPES.has(dbType)
}

export function defaultFormType(dbType) {
  switch (dbType) {
    case 'text':
    case 'json':
      return 'textarea'
...
export function allowedFormTypes(dbType) {
  switch (dbType) {
    case 'string':
...
export function defaultSearchType(dbType) {
...
export function allowedSearchUiTypes(dbType) {
  switch (dbType) {
    case 'date':
...
export function defaultSearchUiType(dbType) {
...
export function normalizeFieldControls(field) {
  ...
  if (!formAllowed.includes(formType)) {
    formType = defaultFormType(dbType)
  }
  let searchUi = field.search_ui_type || ''
  if (!searchUiAllowed.includes(searchUi)) {
