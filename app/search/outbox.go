...
// CreateSyncOutbox 写入同步 outbox。
func CreateSyncOutbox(orderID uint, orderNo, op string, payload map[string]any) {
	...
	if err := appfacades.OrmQuery(context.Background()).Table(OutboxTable).Create(record); err != nil {
...
func findLatestPendingOutboxID(orderID uint, op string) uint {
	...
	if err := appfacades.OrmQuery(context.Background()).Table(OutboxTable).
...
func updateOutboxByID(id uint, data map[string]any) {
	...
	_, _ = appfacades.OrmQuery(context.Background()).Table(OutboxTable).Where("id", id).Update(data)
}

// ListOutboxRetryable 列出可重试 outbox。
func ListOutboxRetryable(limit, maxAttempts int) ([]OutboxRecord, error) {
	...
	err := appfacades.OrmQuery(context.Background()).Table(OutboxTable).
...
func MarkOutboxFailedByID(id uint, errMsg string) {
	...
	var attempts int
	_ = appfacades.OrmQuery(context.Background()).Table(OutboxTable).Where("id", id).Pluck("attempts", &attempts)
	updateOutboxByID(id, map[string]any{
...
func CountOutboxBacklog() (pending int64, failed int64) {
	...
	pending, _ = appfacades.OrmQuery(context.Background()).Table(OutboxTable).
		Where("status", outboxStatusPending).Count()
	failed, _ = appfacades.OrmQuery(context.Background()).Table(OutboxTable).
