...
type OperationLogService interface {
	...
	// ExportOlderThan exports logs older than N days without deleting (compliance export).
	ExportOlderThan(days int) (exportID uint, err error)
...
// ExportOlderThan exports operation logs older than N days without deleting.
func (s *OperationLogServiceImpl) ExportOlderThan(days int) (uint, error) {
	if days <= 0 {
		days = constants.DefaultCleanLogDays
	}
	cutoffTime := time.Now().AddDate(0, 0, -days)
	...
	exportRecord := models.Export{
		AdminID: adminID,
		Type:    models.ExportTypeOperationLogsExport,
		Status:  models.ExportStatusProcessing,
		Disk:    disk,
	}
	if err := appfacades.OrmQuery(s.ctx).Create(&exportRecord); err != nil {
	...
	filename := fmt.Sprintf("operation_logs_export_%d_%s.csv", exportRecord.ID, time.Now().Format("20060102_150405"))
	exportService := &ExportServiceImpl{
		disk:   disk,
		path:   "exports",
		format: "csv",
