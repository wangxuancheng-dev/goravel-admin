...
var (
	...
	ErrFlexibleScheduleNotFound        = NewBusinessError("flexible_schedule_not_found", "可配置定时任务不存在")
	ErrFlexibleScheduleCronInvalid     = NewBusinessError("flexible_schedule_cron_invalid", "Cron 表达式无效")
	ErrFlexibleScheduleHandlerInvalid  = NewBusinessError("flexible_schedule_handler_invalid", "不支持的任务处理器")
	...
	ErrTenantStorageNotConfigured  = NewBusinessError("tenant_storage_not_configured", "租户自定义对象存储未配置完整")
	ErrTenantStorageSecretCorrupt  = NewBusinessError("tenant_storage_secret_corrupt", "租户对象存储密钥密文无效，请重新设置")
	ErrTenantStorageInvalidDriver  = NewBusinessError("tenant_storage_invalid_driver", "租户自定义存储驱动无效，仅支持 s3/oss/cos/minio")
	...
	ErrTenantDomainInvalid       = NewBusinessError("tenant_domain_invalid", "invalid custom domain")
	ErrTenantDomainTaken         = NewBusinessError("tenant_domain_taken", "custom domain already bound")
	ErrTenantDomainNotFound      = NewBusinessError("tenant_domain_not_found", "custom domain not found")
