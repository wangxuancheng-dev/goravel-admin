...
| 支付网关驱动 | `app/services/payment_gateway_*.go` | 每渠道一文件 + `RegisterPaymentGateway`；`Notify` 只走 `ApplyPaidResult`。文件过多（约 8～10+）时**仅搬家驱动**到 `app/payment/gateways/`，注册表与编排仍留 services。详见 [支付参考](/advanced/payments) §6 |
